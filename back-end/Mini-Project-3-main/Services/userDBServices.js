...
//     return new Promise((resolve, reject) =>{
//         let sqlQuery= `INSERT INTO eventdetails VALUES (${ID}, "${Competition}", ${Players}, ${Rounds}, "${TopCut}")`;
//         sql.query(sqlQuery, (err,result, field) => {
//             if(err) return reject(err);
...
//     return new Promise((resolve, reject) =>{
//         let sqlQuery= `UPDATE eventdetails SET Competition = "${newName}", Players = "${newPlayers}", Rounds = ${newRounds}, TopCut = "${newTopCut}" WHERE ID = ${ID}`; //ensure SQL command parameters match variable names defined in function
